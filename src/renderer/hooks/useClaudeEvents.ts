...
export function useClaudeEvents() {
  ...
  useEffect(() => {
    ...
    const unsubEvent = window.clod.onEvent((tabId, event) => {
    ...
    const unsubStatus = window.clod.onTabStatusChange((tabId, newStatus, oldStatus) => {
      handleStatusChange(tabId, newStatus, oldStatus)
    })

    const unsubError = window.clod.onError((tabId, error) => {
      handleError(tabId, error)
    })

    const unsubSkill = window.clod.onSkillStatus((status) => {
      if (status.state === 'failed') {
        console.warn(`[CLOD] Skill install failed: ${status.name} — ${status.error}`)
      }
    })
  ...
  // Note: window.clod.start() is called via sessionStore.initStaticInfo() in App.tsx.
